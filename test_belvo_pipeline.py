...
def _make_transactions(
    ...
    overdraft_days: list[int] | None = None,
) -> list[dict]:
    """
...
_TEST_BELVO_BASE = "https://belvo.test.invalid"
_TEST_BELVO_AUTH = ("test-only-secret-id", "test-only-secret-key")


def _mock_post(url, auth=None, json=None, timeout=None, **_):
    """Intercepts requests.post inside olin/belvo.py"""
    if auth != _TEST_BELVO_AUTH:
        raise AssertionError("Mocked Belvo POST did not use test-only credentials")
    if url not in {
        f"{_TEST_BELVO_BASE}/api/links/",
        f"{_TEST_BELVO_BASE}/api/transactions/",
    }:
        raise AssertionError(f"Unexpected mocked Belvo POST target: {url}")
    mock = MagicMock()
...
def _mock_get(url, auth=None, params=None, timeout=None, **_):
    if auth != _TEST_BELVO_AUTH:
        raise AssertionError("Mocked Belvo GET did not use test-only credentials")
    if url != f"{_TEST_BELVO_BASE}/api/institutions/":
        raise AssertionError(f"Unexpected mocked Belvo GET target: {url}")
    mock = MagicMock()
    mock.status_code = 200
...
def _print_result(res) -> None:
    ...
    print(
        f"  DECISION: {decision_color.get(res.decision.value, '')} {res.decision.value}"
    )
    ...
    if res.approved_amount_mxn > 0:
        ...
        print(
            f"  Fixed cost: MXN {res.pricing_fixed_cost_mxn:>7,.0f}  (60 days, 2 payments)"
        )
    print()
    print("  SIGNALS:")
    for s in res.signals:
        if s.available:
            ...
            src = (
                "Belvo ✓" if s.name == "bank_cash_flow" and not s.fallback_used else ""
            )
            print(
                f"    {s.name:28s}  {s.raw_score:5.1f}  w={s.effective_weight:.2f}  {src}{fb}"
            )
...
def run_tests():
    ...
    with patch.dict(
        "os.environ",
        {
            "BELVO_SECRET_ID": _TEST_BELVO_AUTH[0],
            "BELVO_SECRET_KEY": _TEST_BELVO_AUTH[1],
            "BELVO_BASE_URL": _TEST_BELVO_BASE,
        },
    ):
        from olin import belvo as belvo_module

        with (
            patch.object(belvo_module, "BELVO_BASE", _TEST_BELVO_BASE),
            patch("requests.post", side_effect=_mock_post),
            patch("requests.get", side_effect=_mock_get),
        ):
            # Case A: fetch from existing link
            bank_healthy = belvo_module.fetch_bank_data(
                "fake-link-ofmock-0001", days=90
            )

            # Swap transactions for stressed profile
            with patch(f"{__name__}._TRANSACTIONS", _TRANSACTIONS_STRESSED):
                bank_stressed = belvo_module.fetch_bank_data(
                    "fake-link-ofmock-0002", days=90
                )
    ...
    print(
        f"│  deposit_regularity     : {bank_healthy.deposit_regularity}  (1.0 = perfectly consistent)"
    )
    print(f"│  overdrafts_90d         : {bank_healthy.overdrafts_90d}")
    print(
        f"│  balance_trend_90d      : {bank_healthy.balance_trend_90d:+.3f}  (-1 = declining, +1 = growing)"
    )
    print("└────────────────────────────────────────────────────────────────┘")
    print()
    ...
    print(
        f"│  deposit_regularity     : {bank_stressed.deposit_regularity}  (1.0 = perfectly consistent)"
    )
    ...
    base_fmcg = FMCGData(
        ...
        missed_weeks_last_12=1,
        distributor_confirmed=True,
        trend_3m=0.1,
    )
    base_tenure = TenureData(
        years_on_google_maps=7, years_in_imss=5, address_consistent=True
    )
