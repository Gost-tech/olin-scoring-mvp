...
The Phase 1 migration explicitly aborts if the Phase 0 case or event store contains rows. Authenticated actor provenance cannot be reconstructed honestly, so Phase 1 neither fabricates a production authentication record nor deploys a database that contains permanently unsnapshotable cases. Because Phase 0 forces RLS, the global cutover check and owner-scoped DDL deliberately require a controlled superuser migration connection. The migration temporarily uses `SET SESSION AUTHORIZATION olin_investigator_owner` and then resets it; it never grants owner membership to the migration principal or runtime. Runtime roles remain `NOSUPERUSER`, `NOCREATEROLE`, `NOBYPASSRLS`, non-owner, and directly unable to mutate tables. Deployment must prove the pre-pilot Phase 0 store is empty; handling real legacy rows would require a separately reviewed cutover.
