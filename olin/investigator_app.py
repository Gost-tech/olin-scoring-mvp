...
function values(items,cls){return items.map(x=>`<div class="card ${cls}"><span class="label">${esc(x.value_type)} · ${esc(x.quantity)}</span><div class="value">${esc(x.value)} ${esc(x.unit)}</div><p>${esc(x.period_start)} — ${esc(x.period_end)}</p><small>Meaning: ${esc(x.dimensional_scope)} · rule ${esc(x.rule_id)}</small><p>${esc((x.assumptions||[]).join('; '))}</p><details><summary>Evidence and provenance</summary><small>${refs(x)}</small></details></div>`).join('')}
